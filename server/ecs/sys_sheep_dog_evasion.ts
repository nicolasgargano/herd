import {Settings, dogsQuery, sheepQuery, sheepMovementQuery} from "./components"
import {World} from "@javelin/ecs"
import {Clock} from "@javelin/hrtime-loop"
import {Vector2} from "three"

export const sys_sheep_dog_evasion = (boidSettings: Settings, world: World<Clock>) => {
  sheepMovementQuery((sheepId, [sheep, sheepPos, sheepMovement]) => {
    ...
    dogsQuery((dogId, [, dogPos]) => {
      ...
      if (dogToThis.lengthSq() < boidSettings.alertRadius ** 2) {
        dogs++
        const inverselyProportional = dogToThis
          .divideScalar(Math.max(Math.pow(dogToThis.length(), 2), 0.001 ))
    ...
    if (accumulatedDogDistancing.length() === 0) {
      ...
      const weightedSteerForce = steerForce.multiplyScalar(boidSettings.dogEvasionWeight);
