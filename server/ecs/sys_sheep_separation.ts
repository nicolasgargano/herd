...
export const sys_sheep_separation = (boidSettings: Settings, world: World<Clock>) => {
  sheepMovementQuery((sheepId, [sheep, pos, movement]) => {
    if (sheep.neighbors.length > 0) {
      ...
      for (const neighborId of sheep.neighbors) {
        ...
        if (neighborPosition) {
          const neighborToThis = (pos as Vector2).clone()
            .sub(neighborPosition as Vector2)

          const inverselyProportional = neighborToThis
            .divideScalar(Math.pow(neighborToThis.length(), 2) || 0.00001)

          accumulatedDistancing.add(inverselyProportional)
        }
      }

      const averageDistancing = accumulatedDistancing.divideScalar(sheep.neighbors.length)
      ...
      const weightedSteerForce = steerForce.multiplyScalar(boidSettings.separationWeight);
